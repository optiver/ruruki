...
def make_query(query):
    ...
    {
        'action': 'return',
        'return_item_grouped': [{'alias': 'a'}],
        'vertices': [
            {'alias': 'a', 'properties': {'name': 'test'}},
            # {'alias': 'b'}
        ],
        'where_action': {
            'action': 'where',
    ...
    q = {
        "$and": [
            {"$vertices": {"name": "test1"}},
            {"$vertices": {"name__eq": "blah"}},
        ]
    }

    From pyparsing:
    'vertices': [
        {'alias': 'v', 'properties': {'name': 'test1'}},
        {'alias': 'b'}
    ]
    ...
    q = {
        "$or": [
            {"$vertices": {"name": "test1"}},
            {"$vertices": {"name__contains": "s"}},
        ]
    }
    """
    def create_vertex_query(d):
        alias = d.get("alias")
        labels = d.get("labels")
        properties = d.get("properties", {})

        if labels:
            properties["label"] = labels[0]

        key = "$vertices|{}".format(alias) if alias else "$vertices"

        return {key: properties}

    q = {}
    if "where_action" in query:
        q = {"$and": []}
        for vertex in query.get("vertices", []):
            if "alias" not in vertex:
                raise SyntaxError(
                    "Can not perform a 'where' predicate with no alias."
                )
            vq = create_vertex_query(vertex)

            w_alias =query["where_action"]["alias"]
            v_alias =vertex.get("alias")
            if w_alias != v_alias:
                raise SyntaxError(
                    "Where predicate alias mismatch."
                )

            key = query["where_action"]["key"]
            operation = query["where_action"]["operation"]
            value = query["where_action"]["value"]
            cvp = {
                "$vertices": {
                    "{}__{}".format(key, operation): value
                }
            }
